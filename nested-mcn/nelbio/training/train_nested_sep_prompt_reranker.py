...
from nelbio.models.nested_sep_reranker import NestedScoreRerankNet
...
def parse_args():
    ...
    parser.add_argument("--fixed_flat_dense_weight", action="store_true")
    parser.add_argument("--fixed_sep_dense_weight", action="store_true")
    parser.add_argument("--fixed_flat_sparse_weight", action="store_true")
...
def main(args):
    ...
    sep_pooling = args.sep_pooling
    logging.info(f"Using sep pooling: {sep_pooling}")

    model = NestedScoreRerankNet(bert_encoder=biosyn.get_dense_encoder(),
                                 tokenizer=biosyn.get_dense_tokenizer(),
                                 sparse_encoder=biosyn.get_sparse_encoder(),
                                 learning_rate=args.learning_rate,
                                 weight_decay=args.weight_decay,
                                 sparse_weight=args.initial_sparse_weight,
                                 dense_encoder_score_type=args.dense_encoder_score_type,
                                 flat_dense_weight=args.initial_flat_dense_weight,
                                 sep_dense_weight=args.initial_sep_dense_weight,
                                 sep_pooling=sep_pooling,
                                 fixed_flat_dense_weight=args.fixed_flat_dense_weight,
                                 fixed_flat_sparse_weight=args.fixed_flat_sparse_weight,
                                 fixed_sep_dense_weight=args.fixed_sep_dense_weight
                                 ).to(device)
    ...
    for epoch in range(1, args.epoch + 1):
        ...
        LOGGER.info(f"loss/train_per_epoch={train_loss}/{epoch}, sparse: {model.sparse_weight.item():.5f}, "
                    f"flat_dense: {model.flat_dense_weight.item():.5f}, sep_dense: {model.sep_dense_weight.item():.5f}")
