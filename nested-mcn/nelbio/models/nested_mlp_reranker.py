...
class MLPReranker(nn.Module, AbstractRerankNet):
    def __init__(self, bert_encoder, tokenizer, sparse_encoder, learning_rate, weight_decay,
                 sparse_weight, dense_encoder_score_type, mlp_lr,
                 sep_pooling="cls",):
        LOGGER.info(f"RerankNet! learning_rate={learning_rate} weight_decay={weight_decay}"
                    f"{sparse_weight}")

        super(MLPReranker, self).__init__()
        ...
        self.optimizer = optim.Adam([
            ...
            {'params': self.out.parameters(), 'lr': mlp_lr, 'weight_decay': 0}
        ],
    ...
    def forward(self, query_token_input, candidate_input, sep_context_input, nested_sep_candidate_input,
        ...
        b, k, h = candidate_emb.size()
        assert query_emb.size() == (b, h)
        ...
        sep_context_emb_repeated = sep_context_emb.unsqueeze(1).expand((b, k, h))
        assert sep_context_emb_repeated.size() == (b, k, h)

        sep_context_cand_emb = torch.cat((sep_context_emb_repeated, sep_candidate_emb), dim=-1)
        assert sep_context_cand_emb.size() == (b, k, h * 2)
        # b, k
        sep_dense_score = self.out(sep_context_cand_emb).squeeze(-1)
        ...
        score = self.sparse_weight * candidate_s_scores + flat_dense_score + sep_dense_score
    ...
    def save_model(self, path):
        ...
        out_path = os.path.join(path, 'out.pt')
    ...
    def load_model(self, model_name_or_path):
        ...
        self.out.load_state_dict(torch.load(out_path))
