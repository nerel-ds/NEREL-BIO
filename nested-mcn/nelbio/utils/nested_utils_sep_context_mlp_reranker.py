    ...
    with open(log_file_path, "w+", encoding="utf-8") as out_file, \
        ...
        for batch in eval_dataloader:
            ...
            with (torch.no_grad()):
                ...
                flat_score_matrix = get_flat_score_matrix(score_mode, sparse_weight, 1.,
                                                          sparse_score_matrix,
                ...
                b, k, h = sep_candidate_emb.size()
                sep_context_emb_repeated = sep_context_emb.unsqueeze(1).expand((b, k, h))
                sep_context_cand_emb = torch.cat((sep_context_emb_repeated, sep_candidate_emb), dim=-1).to(device)


                sep_dense_score_matrix = model.out(sep_context_cand_emb).squeeze(-1).detach().cpu().numpy()
                ...
                sum_score_matrix = sparse_weight * sub_sparse_score_matrix + 1. * \
                                   sub_flat_dense_score_matrix + 1. * sep_dense_score_matrix
...
def evaluate_nested_flat_reranker(eval_dataloader: DataLoader, model, biosyn, eval_dictionary, topk, score_mode,
