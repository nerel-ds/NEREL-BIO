...
mkdir -p results/eval_mlp_reranker_frozen_biosyn/random_split_dev/e_${ep}
...
--data_dir "data/biosyn_format/random_split/dev" \
--use_cuda \
--topk 20 \
--score_mode "hybrid" \
--query_max_length 32 \
--context_max_length 96 \
--output_dir results/eval_mlp_reranker_frozen_biosyn/random_split_dev/e_${ep} \
--filter_composite \
--filter_duplicate \
--drop_cuiless \
--save_predictions
